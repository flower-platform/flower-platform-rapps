...
public: 
	...
	AppGen()
		: httpServer(80)
		, ethernetNetworkAdapter("192.168.100.252", "00:01:02:03:04:05")
	{};

	int testLengthFunction(String text) {
        return ((String)text).length();

	}

	String testIsEmptyFunction(String text) {
	...
	String testJoinFunction(String first, String second) {
	...
	String testTrimFunction(String text) {
        return getTrimmedString (((String)text));
	...
	void httpServer_onCommandReceived(HttpCommandEvent* event) {
		...
		if (strcmp_P(command, PSTR("")) == 0) {
			...
			out.flush();
			return;
		} else if (strcmp_P(command, PSTR("testLengthFunction")) == 0) {
			urlFound = true;
			char textBuf[32];
			event->server->getStringParameterValue(event->url, "text", textBuf);
			String text = textBuf;
			returnValueJson = returnValueBuf;
			itoa(testLengthFunction(text), returnValueJson, 10);
		} else if (strcmp_P(command, PSTR("testIsEmptyFunction")) == 0) {
			urlFound = true;
			char textBuf[32];
			event->server->getStringParameterValue(event->url, "text", textBuf);
			String text = textBuf;
			...
			strcat(returnValueJson, testIsEmptyFunction(text).c_str());
			strcat(returnValueJson, "\"");
