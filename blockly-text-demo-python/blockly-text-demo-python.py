...
from FlowerPlatformRuntime.HttpServer import HttpServer
...
class AppGen :
	...
	def httpServer1_onCommandReceived(self, event) :
		...
		if (event.url == "") :
			...
			urlFound = True
			text = event.parameters.get("text")[0]
			returnValueJson = '"' + self.testIsEmptyFunction(text) + '"'
		elif (event.url == "testCompareFunction") :
			urlFound = True
			first = event.parameters.get("first")[0]
			second = event.parameters.get("second")[0]
			returnValueJson = '"' + self.testCompareFunction(first, second) + '"'
		elif (event.url == "testLowerCaseFunction") :
			urlFound = True
			text = event.parameters.get("text")[0]
			returnValueJson = '"' + self.testLowerCaseFunction(text) + '"'
		elif (event.url == "testJoinFunction") :
			urlFound = True
			first = event.parameters.get("first")[0]
			second = event.parameters.get("second")[0]
			returnValueJson = '"' + self.testJoinFunction(first, second) + '"'
		elif (event.url == "testUppercaseFunction") :
			urlFound = True
			text = event.parameters.get("text")[0]
			returnValueJson = '"' + self.testUppercaseFunction(text) + '"'
		elif (event.url == "testReplaceFunction") :
			urlFound = True
			text = event.parameters.get("text")[0]
			segment = event.parameters.get("segment")[0]
			replacement = event.parameters.get("replacement")[0]
			returnValueJson = '"' + self.testReplaceFunction(text, segment, replacement) + '"'
		elif (event.url == "testSegmentTextFunction") :
			urlFound = True
			text = event.parameters.get("text")[0]
			start = int(event.parameters.get("start")[0])
			len = int(event.parameters.get("len")[0])
			returnValueJson = '"' + self.testSegmentTextFunction(text, start, len) + '"'
		elif (event.url == "testStartsAtTextFunction") :
			urlFound = True
			text = event.parameters.get("text")[0]
			...
			urlFound = True
			text = event.parameters.get("text")[0]
			returnValueJson = '"' + self.testTrimFunction(text) + '"'
		elif (event.url == "testContainsTextFunction") :
			urlFound = True
			text = event.parameters.get("text")[0]
			piece = event.parameters.get("piece")[0]
			returnValueJson = '"' + self.testContainsTextFunction(text, piece) + '"'
		...
		if returnValueJson : 
			event.client.wfile.write('"__RESULT":%s' % returnValueJson)
		event.client.wfile.write("}")
	
	def getCommandDescriptorsAsJson(self) :
		json = '['
		json += "{\"label\":\"testLengthFunction\", \"command\":\"testLengthFunction\", \"parameters\":\"text:String\"}"
		json += ",{\"label\":\"testIsEmptyFunction\", \"command\":\"testIsEmptyFunction\", \"parameters\":\"text:String\"}"
		json += ",{\"label\":\"testCompareFunction\", \"command\":\"testCompareFunction\", \"parameters\":\"first:String; second:String\"}"
		json += ",{\"label\":\"testLowerCaseFunction\", \"command\":\"testLowerCaseFunction\", \"parameters\":\"text:String\"}"
		json += ",{\"label\":\"testJoinFunction\", \"command\":\"testJoinFunction\", \"parameters\":\"first:String; second:String\"}"
		json += ",{\"label\":\"testUppercaseFunction\", \"command\":\"testUppercaseFunction\", \"parameters\":\"text:String\"}"
		json += ",{\"label\":\"testReplaceFunction\", \"command\":\"testReplaceFunction\", \"parameters\":\"text:String; segment:String; replacement:String\"}"
		json += ",{\"label\":\"testSegmentTextFunction\", \"command\":\"testSegmentTextFunction\", \"parameters\":\"text:String; start:int; len:int\"}"
		json += ",{\"label\":\"testStartsAtTextFunction\", \"command\":\"testStartsAtTextFunction\", \"parameters\":\"text:String; piece:String\"}"
		json += ",{\"label\":\"testTrimFunction\", \"command\":\"testTrimFunction\", \"parameters\":\"text:String\"}"
		json += ",{\"label\":\"testContainsTextFunction\", \"command\":\"testContainsTextFunction\", \"parameters\":\"text:String; piece:String\"}"
		json += "]"
		return json
